    ...
    int $perPage = 20
        ...
        if (!empty($filters['category_id'])) {
            ...
            $where .= " AND EXISTS (
                ...
                WHERE ac.article_id = a.id AND ac.category_id IN (" . implode(',', $placeholders) . ")
            )";
        }
        ...
        if (!empty($filters['tag_id'])) {
            ...
            $where .= " AND EXISTS (
                ...
                WHERE at.article_id = a.id AND at.tag_id IN (" . implode(',', $placeholders) . ")
            )";
        }
        ...
        if (!empty($filters['status']) && in_array($filters['status'], \App\Lib\ArticleStatus::all(), true)) {
